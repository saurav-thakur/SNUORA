...
def detail(request, id):
    ...
    if request.method == 'POST':
        ...
        det = answerData['detail'].value()

        cleaned_text = [clean_text(det)]
 
        seq = load_tokenizer.texts_to_sequences(cleaned_text)
        padded = sequence.pad_sequences(seq, maxlen=300)
        pred = load_model.predict(padded)
        if pred > 0.5:
            messages.error(request, 'Abusive content detected.')

        elif answerData.is_valid():
...
def save_comment(request):
    ...
    # print(test)
        ...
        if pred > 0.5:
            messages.error(request, 'Abusive comment detected.')
...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if regForm.is_valid():
            ...
            messages.warning(request, "Poor Password")
...
def ask_form(request):
    ...
    if request.method == 'POST':
        ...
        text = questForm['detail'].value()
        tg = questForm['tags'].value()
        ttle = questForm['title'].value()



        cleaned_title = [clean_text(ttle)]
    # print(test)
        seq_title = load_tokenizer.texts_to_sequences(cleaned_title)
        padded_title = sequence.pad_sequences(seq_title, maxlen=300)
        pred_title = load_model.predict(padded_title)

        cleaned_tag= [clean_text(tg)]
    # print(test)
        seq_tag = load_tokenizer.texts_to_sequences(cleaned_tag)
        padded_tag = sequence.pad_sequences(seq_tag, maxlen=300)
        pred_tag = load_model.predict(padded_tag)
